#!/usr/bin/env python3
"""
Density distribution heatmap in z cross-section
"""
...
parser = argparse.ArgumentParser(description='Create grid density heatmap')
...
# density column name: c_prop_grid[1] is nrho
nrho_col = [c for c in first_df.columns if "c_prop_grid[1]" in c][0]

def density_hist(df):
    """Return 2D array of mean density for |z| < delta_z, binned on (x,y) using cell centres."""
    xc = df["xc"].to_numpy()
    yc = df["yc"].to_numpy()
    zc = df["zc"].to_numpy()
    ...
    # get density (number density)
    nrho = df[nrho_col].to_numpy()[mask]
    xs, ys = xc[mask], yc[mask]

    sum_n, _, _ = np.histogram2d(xs, ys, bins=[x_edges, y_edges], weights=nrho)
    cnt_n, _, _ = np.histogram2d(xs, ys, bins=[x_edges, y_edges])
    with np.errstate(invalid='ignore', divide='ignore'):
        mean_n = np.where(cnt_n > 0, sum_n / cnt_n, np.nan)
...
cbar = fig.colorbar(im, ax=ax, label="n (#/m³)")
title = ax.set_title("")
ax.set_xlabel("x (m)")
ax.set_ylabel("y (m)")
...
tstep = extract_tstep_from_input("in.distributor3D")
...
def update(i):
    ...
    title.set_text(f"density heatmap |z| ≤ {slice_frac:.2f}H  |  time = {step * tstep:.2e} s")
    return im, title

ani = FuncAnimation(fig, update, frames=len(timesteps), init_func=init, blit=False, interval=200)
ani.save("outputs/grid_density_heatmap.mp4", fps=30, dpi=300)
print("Saved outputs/grid_density_heatmap.mp4")
