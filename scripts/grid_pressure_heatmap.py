...
# pressure column name: c_prop_grid[1] is nrho, need to calculate P = nrho * kB * T
kB = 1.380649e-23  # J/K
T = 300.0  # K (from simulation)
...
def pressure_hist(df):
    ...
    # calculate pressure: P = nrho * kB * T
    nrho = df[nrho_col].to_numpy()[mask]
    pressure = nrho * kB * T  # Pa
    xs, ys = xc[mask], yc[mask]

    sum_p, _, _ = np.histogram2d(xs, ys, bins=[x_edges, y_edges], weights=pressure)
...
print(f"vmin={vmin:.2f} Pa, vmax={vmax:.2f} Pa")
...
cbar = fig.colorbar(im, ax=ax, label="P (Pa)")
title = ax.set_title("")
ax.set_xlabel("x (m)")
ax.set_ylabel("y (m)")
